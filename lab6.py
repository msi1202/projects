...
def has_kitty_gene(dna):
    ...
    False
    """
    required_gene=link('C',link('A',link('T',link('C',link('A',link('T'))))))

    return has_sublist(dna, required_gene)
...
def count_change(amount, denominations):
    ...
    if amount==0:
        return 1
    updatedAmount=amount - first(denominations)
    beginning=count_change(updatedAmount, denominations)
    remaining=count_change(amount, denominations[1])
