## Lab 8: Trees ##
# Written by Mohamed Sameer Imam
# imamms
# M15124129
...
def make_pytunes(username):
    ...
    return tree(username, [
        tree('pop', 
             [tree('justin bieber', 
                   [tree('single', 
                         [tree('what do you mean?')])]),tree('2015 pop mashup')]),tree('trance', [tree('darude', [tree('sandstorm')])])])
...
def num_songs(t):
    ...
    if is_leaf(t):
        ...
        count = 0
        for remaining in branches(t):
            count += num_songs(remaining)
        return count
...
def add_song(t, song, category):
    ...
    if root(t) == category:
        ...
        new_tree = []
        for remaining in branches(t):
            new_tree.append(add_song(remaining, song, category))
        return tree(root(t), new_tree)
...
def delete(t, target):
    ...
    temp_tree = []
    for remaining in branches(t):
        if root(remaining) != target:
            temp_tree.append(delete(remaining, target))

    return tree(root(t), temp_tree)
